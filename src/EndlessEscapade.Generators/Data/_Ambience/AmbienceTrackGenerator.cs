        ...
        initializationContext.RegisterSourceOutput(
            ...
            static (sourceContext, content) => {
                sourceContext.AddSource(
                    $"{content.Name}.g.cs",
                    SourceText.From(GenerateAmbienceTrack(content.Name, in content.Data), Encoding.UTF8)
                );
            }
        );
    }

    private static string GenerateAmbienceTrack(string name, in AmbienceTrackData data) {
...
{{
	public override SoundStyle Sound {{ get; }} = new(""{data.SoundStyleData.SoundPath}"", SoundType.Ambient) {{
